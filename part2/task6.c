    ...
    for (int loop_num = 0; loop_num < size; loop_num ++){
      if (rank == 0){
        ...
        if (loop_num == 0){
          ...
          comm_start = MPI_Wtime();
          // need to wait for data before begin computation
          ...
          total_compute_time += compute_end - compute_start;
          comm_start = MPI_Wtime();
          // need to make sure new data is received
        ...
        compute_end = MPI_Wtime();
        total_compute_time += compute_end - compute_start;
        // before sending new data, need to confirm old data is received.
        if (loop_num != 0){
          comm_start = MPI_Wtime();
        ...
        // set the starting point based on current loop_num
        int iC = t*lines_per_block*lines_per_block;
        int count = 0;
        compute_end = MPI_Wtime();
        total_compute_time += compute_end - compute_start;
        comm_start = MPI_Wtime();
        // need to wait for data until received each round before starting calculation
        if (loop_num != 0){
          MPI_Wait(&recv_request, &status);
        }
        else{
